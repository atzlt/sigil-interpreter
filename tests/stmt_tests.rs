...
fn math_registry() -> FunctionRegistry {
    ...
    fn add(args: &[Value]) -> Value {
        let a = match &args[0] { Value::Number(n) => *n, _ => 0.0 };
        let b = match &args[1] { Value::Number(n) => *n, _ => 0.0 };
        Value::Number(a + b)
    }
...
fn test_program_let_expr_return() {
    assert_eq!(
        run_program("let x = 1 + 2 * 3; x + 4; return;"),
        Value::Nil
    );
}
...
fn test_program_return_nil_after_lets() {
    assert_eq!(
        run_program("let a = 1; let b = 2; return;"),
        Value::Nil
    );
}
